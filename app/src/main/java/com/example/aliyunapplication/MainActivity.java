...
public class MainActivity extends AppCompatActivity {
        ...
        HiveMQUtils.setHandler(new Handler(Looper.getMainLooper(), msg -> {
            if (msg.what == 3 && msg.obj != null) {
                ...
                try {
                    ...
                    if (json.has("stall_1")) {
                        int status = json.getInt("stall_1");
                        slot1.status = status;
                        if (status == ParkingSlot.STATUS_OCCUPIED) slot1.startTs = System.currentTimeMillis();
                        if (json.has("number")) slot1.plate = json.getString("number");
                    }
                    if (json.has("stall_2")) {
                        int status = json.getInt("stall_2");
                        slot2.status = status;
                        if (status == ParkingSlot.STATUS_OCCUPIED) slot2.startTs = System.currentTimeMillis();
                        if (json.has("number")) slot2.plate = json.getString("number");
                    }
                    updateSlotViews();
    ...
    public void onSendButtonClicked(View view) {
        ...
        try {
            ...
            obj.put("number", car);
            updateSlotViews();
            HiveMQUtils.publish(obj.toString());
    ...
    private void saveParkRecord(String plate, String duration, double fee) {
        ...
        editor.apply();
        Toast.makeText(this, "缴费记录已保存", Toast.LENGTH_SHORT).show();
    }
    ...
    private void debugSetSlot(ParkingSlot slot, int status, String plate) {
        ...
        try {
            ...
            HiveMQUtils.publish(msg.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
